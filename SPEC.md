...
| D7 | **Handlebars-compatible templating subset** via a vendored Go Handlebars engine; templates compiled at registration; unknown helpers → 422 | Preserves D2 for the very common templated-stub case | Go `text/template` (breaks every existing templated stub); no templating |
...
- Vendored fork of a Go Handlebars implementation (`mailgun/raymond` lineage) under `internal/handlebars` — we own it: only allowlisted helpers registered, no filesystem/partials-from-disk, deterministic behavior.
...
internal/jsonpath/        JSONPath seam (wraps chosen lib)
internal/regexx/          RE2/regexp2 seam, timeouts
internal/handlebars/      vendored engine fork
...
**Dependency policy** (allowlist; anything else needs a PR discussion): `gocb/v2`, `dlclark/regexp2`, vendored handlebars fork, one JSONPath lib (§6.7), `prometheus/client_golang`, `google/uuid`, `segmentio/ksuid`, `golang.org/x/net` (h2c), test-only: `testcontainers-go`, `stretchr/testify`, `gopkg.in/yaml.v3` (e2e corpus parsing). Stdlib for everything else (`log/slog`, `encoding/json`, `net/http`). No web framework, no DI framework, no config framework beyond trivial binding.
...
### M3 — Templating *(M)*
**First task: resolve the WM activation-matrix [DH] via the harness** — it decides what `wm-compat` means (§10.1). Then: vendor+prune handlebars engine, request model, helper allowlist with per-helper corpus cases, registration-time validation, render path + caps. JSONPath lib decision finalized via conformance mini-corpus (shared with matchers). **Exit**: S3 met; template compat cases green; unknown-helper 422s verified.
...
| JSONPath dialect gaps (Jayway quirks) | Body matching differs | Conformance mini-corpus gates lib choice; seam allows swap; unsupported syntax → 422 at registration |
...
- **Vendored handlebars fork** (`internal/handlebars`, §10.1): upstream is MIT. The upstream license text and **both** upstream copyright lines (aymerick + mailgun fork) are preserved inside the vendored tree; our modifications are Apache-2.0. Vendored in-tree code is invisible to `go-licenses` (it scans modules), so its attribution is maintained **manually** in a curated section of `THIRD_PARTY_LICENSES`.
- **Dependency license gate**: CI (`go-licenses`) verifies every module, transitively, is on the allowlist — Apache-2.0, MIT, BSD-2/3-Clause, ISC — and regenerates `THIRD_PARTY_LICENSES` (module-derived part; the vendored section is curated). Copyleft (GPL/LGPL/AGPL/MPL) is denied for the shipped binary's module graph with no exceptions; a transitive **test-only** dep outside the allowlist requires a reviewed entry in `licenses_exceptions.yaml` with justification. The §18 allowlist is verified compatible today: gocb (Apache-2.0), regexp2 (MIT), raymond lineage (MIT), prometheus/client_golang (Apache-2.0), google/uuid (BSD-3), ksuid (MIT), x/net (BSD-3), testcontainers-go, testify & yaml.v3 (MIT/MIT/MIT-equivalent, test-only). The M3 JSONPath library choice (§6.7) carries an explicit criterion: its license must be `go-licenses`-**classifiable** and allowlisted — custom-worded license texts classify as Unknown and hard-fail the gate, which disqualifies otherwise-permissive candidates unless resolved.
