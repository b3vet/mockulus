...
4. Java syntax is **translated** before either engine sees it, where the translation is exact. A character class Java defines differently from RE2 is lowered to Java's definition — `\s` and `\S` are `[ \t\n\x0B\f\r]` in Java and omit the vertical tab in RE2, so a pattern using the most common escape in the language means the same thing on both servers. This is policy, not an exception to it: a translation that is exact costs nothing at match time and removes a wrong answer rather than a missing feature, so it is preferred wherever one exists. What remains accepted as divergent is the residue where no exact translation does — semantics that differ for patterns compiling on both engines, such as `\d` and `\w` under Unicode, where Java's definition depends on flags RE2 has no equivalent for. The differential corpus carries the known cases of both, and `internal/regexx/javasyntax.go` is where the translation table lives.
